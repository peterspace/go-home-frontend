...
const MessageForm = (props) => {
  ...
  const autoSendMessage = async () => {
    ...
    `;
    if (transactionData) {
      // socket.emit('stop typing', txInfo?.chatId);
      socket.emit('stop typing', '64ea1ba3eae5c3e9865eac7a');
      try {
        const config = {
          headers: {
        ...
        const { data } = await axios.post(
          ...
          {
            content: newContent,
            // chatId: txInfo?.chatId,
            chatId: '64ea1ba3eae5c3e9865eac7a',
          },
          config
        );
        setNewData(data);
