...
import { updatePrice, updateFromPrice, fetchChainPrice } from '../../api/api';
...
async function getPrice(userData) {
  try {
    const response = await axios.get(
      `https://api.1inch.io/v5.0/${userData?.chainId}/quote?fromTokenAddress=${userData?.fTokenAddress}&toTokenAddress=${userData?.tTokenAddress}&amount=${userData?.validatedValue}&fee=${fee}&gasLimit=3000000`
    );
    if (response.data) {
      return response;
    }
...
async function getPriceCompare(userData) {
  ...
  try {
    ...
    const response = await axios.get(
      `https://api.1inch.io/v5.0/${chainId}/quote?fromTokenAddress=${fToken?.address}&toTokenAddress=${tToken?.address}&amount=${amount}&fee=${fee}&gasLimit=3000000`
    );
    if (response.data) {
...
async function getFromUSDPrice(userData) {
  ...
  try {
    if (fToken?.address === usdtToken.address) {
      ...
      let amount = parseUnits('1', fToken?.decimals.toString());
      const response = await axios.get(
        `https://api.1inch.io/v5.0/${chainId}/quote?fromTokenAddress=${fToken?.address}&toTokenAddress=${usdtToken?.address}&amount=${amount}&fee=${fee}&gasLimit=3000000`
      );
      if (response.data) {
        let rawValue = response.data.toTokenAmount;
...
async function getToUSDPrice(userData) {
  ...
  try {
    if (tToken?.address === usdtToken?.address) {
      ...
      const response = await axios.get(
        `https://api.1inch.io/v5.0/${chainId}/quote?fromTokenAddress=${tToken?.address}&toTokenAddress=${usdtToken?.address}&amount=${amount}&fee=${fee}&gasLimit=3000000`
      );
      if (response.data) {
        let rawValue = response.data.toTokenAmount;
        let value = rawValue / 10 ** usdtToken?.decimals;
...
async function getChainUSDPrice(userData) {
  ...
  try {
    ...
    const response = await axios.get(
      `https://api.1inch.io/v5.0/${chainId}/quote?fromTokenAddress=${addressNative}&toTokenAddress=${usdtToken?.address}&amount=${amount}&fee=${fee}&gasLimit=3000000`
    );
    if (response.data) {
      let rawValue = response.data.toTokenAmount;
      let value = rawValue / 10 ** usdtToken?.decimals;
...
async function approve(userData) {
  try {
    const response = await axios.get(
      `https://api.1inch.io/v5.0/${userData?.chainId}/approve/transaction?tokenAddress=${userData?.fTokenAddress}&amount=${userData?.validatedValue}`
    );
    if (response.data) {
      return response.data;
    }
...
async function fetchChainPriceData(userData) {
  try {
    const response = await fetchChainPrice(userData)

    if (response) {
      console.info('fetchChainPriceData', response);
      return response;
    }
  } catch (error) {
    const message =
...
const fetchFromPrice = async (userData) => {
  const response = await updateFromPrice(userData)

  if (response) {
    console.info('updateFromPrice', response);
    return response;
  }
};
...
const fetchToPrice = async (userData) => {
  const response = await updatePrice(userData);

  if (response) {
    console.info('updateToPrice', response);
    return response;
  }
};
