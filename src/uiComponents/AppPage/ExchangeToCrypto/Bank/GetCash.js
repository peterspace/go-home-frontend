...
export default function BankTransactPage({ walletAddress, chainId, txId }) {
  ...
  async function getbanksTransact(txId) {
    ...
    setDexShare(item.dexShare);
  }

  async function bankGetCash(txId) {
    try {
      let dexContract = new ethers.Contract(
        dexContractAddress,
        dexContractAbi,
      ...
      let transaction = await dexContract.bankReceiveCash(txId, dexShare, {
        gasLimit: 3000000,
      });
  ...
  function handleGetCashSubmit(txId) {
    ...
    window.location.replace('/get-cash/bankpage'); // location back to the bank page
  }
