...
const SwapCrypto = (props) => {
  ...
  const chats = localStorage.getItem('chats')
    ...
    const isBlockchain = localStorage.getItem('isBlockchain')
    ? JSON.parse(localStorage.getItem('isBlockchain'))
    : null;
...
*/
  ...
  //     // managerId: allActiveBanks[0]?._id,
  //     country: 'Russia',
  //     state: 'Moscow',
  //     city: 'Asino',
  //     walletAddress,
  ...
  //========={Fetch transaction Data after user has updated transaction}===========
  const fetchUserTransactionsById = async () => {
    try {
      ...
      const { data } = await axios.get(
        `${BACKEND_URL}/api/transaction/getUserTransactionById/${transactionData?._id}`,
        config
      );
      // setLoading(false);

      if (data) {
        setTransactionData(data); // refetching transaction
      }
    } catch (error) {
  ...
  //After user has been connected with Bank
  ...
  const mainTransactionData = (
    <div className={stylesSwap.frameGroupCustom}>
      ...
      <div className="flex flex-col gap-2">
        ...
        <p className="">{`you send: ${transactionData?.fromValue}`}</p>
        <p className="">{`you get: ${transactionData?.toValue}`}</p>
      </div>
      <>
