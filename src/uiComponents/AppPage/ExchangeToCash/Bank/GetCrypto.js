...
export default function BankTransactPage({ walletAddress, chainId, txId }) {
  ...
  const [dexShare, setDexShare] = useState('');
  ...
  async function getbanksTransact(txId) {
    ...
    setAmount(data.CryptoPrice); // set amount
    setTransactToken(data.token); // set the token
      setTokenDecimals(data.tokenDecimals); // set the decimals
      setDexShare(data.dexShare);
  }


  // Dex receives DexShare
  async function bankGetCrypto(txId, dexShare) {
    ...
    try {
      ...
      let transaction = await dexContract.bankGetCrypto(txId, 
      {
  ...
  function handleGetCryptoSubmit(txId) {
    bankGetCrypto(txId);
    window.location.replace('/bankpage'); // location
  }
    ...
    <div style={{ 'min-height': '100vh' }}>
      ...
      <div className="flex ml-20 mt-20">
        ...
        <div className="text-xl ml-20 space-y-8 text-white shadow-2xl rounded-lg border-2 p-5">
          ...
          <div>
            {currAddress == data.bank && transactions == 'Get Crypto' ? (
              <button
