...
export default function BankTransactPage({ walletAddress, chainId, txId }) {
  ...
  async function bankPayCash(txId) {
    try {
      let dexContract = new ethers.Contract(
        dexContractAddress,
        dexContractAbi,
      ...
      let transaction = await dexContract.bankPayCash(txId, {
        gasLimit: 3000000,
      });
      await transaction.wait();

      alert('You have paid, please wait for the client to confirm!');
      updateMessage('');
    } catch (e) {
    ...
    <div style={{ 'min-height': '100vh' }}>
      ...
      <div className="flex ml-20 mt-20">
        ...
        <div className="text-xl ml-20 space-y-8 text-white shadow-2xl rounded-lg border-2 p-5">
          ...
          <div>
            {currAddress == data.bank && transactions == 'Pay Cash' ? (
              <button
