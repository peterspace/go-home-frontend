...
const SwapCrypto = (props) => {
  ...
  //====={New}======================================
  const txIdL = localStorage.getItem('txId')
    ? JSON.parse(localStorage.getItem('txId'))
    : '';
  const [txId, setTxId] = useState(txIdL);

  console.log({ txId: txId });
  const [newTx, setNewTx] = useState();
  console.log({ newTx: newTx });

  const newBkDataL = localStorage.getItem('newBkData')
    ? JSON.parse(localStorage.getItem('newBkData'))
    : '';
  const [newBkData, setNewBkData] = useState(newBkDataL);

  console.log({ newBkData: newBkData });

  // let sendTransaction = newBkData;
  //       //============={New}=============================================
  //       let event = sendTransaction?.events[0];
  //       let value = event?.args[3];
  //       // let value = event?.args[4];
  //       // let newTxId = value.toNumber();
  //          let newTxId = value;

  //       console.info({newTxId: newTxId});

  const [newBkTxId, setNewBkTxId] = useState();

  console.log({ newBkTxId: newBkTxId });

  const [newGetTx, setNewGetTx] = useState();

  console.log({ newGetTx: newGetTx });
...
*/
  ...
  useEffect(() => {
    localStorage.setItem('newBkData', JSON.stringify(newBkData));
  ...
  async function getTx1() {
    ...
    // let value = event?.args[3];
    // let value = event?.args[4];
    // let value = event?.args[3]?.hex;
    // let newTxId = value.toNumber();
    //  let newTxId = value;

    let value = event?.args[3]?.hex;
    // let value = event?.args[4]?.hex;

    // let newTxId = parseFloat(value)
    // let newTxId = parseFloat(value)
    let newTxId = formatEther(value);

    setNewBkTxId(newTxId);
  ...
  //   let tk = dexContract.on(
  //     'ClientOrder',
  //     (token, client, bank, CryptoPrice, date) => {
  //       console.log('ClientOrder event was emitted');
  //       console.log({ token: JSON.stringify(token) });
  //       console.log({ client: JSON.stringify(client) });
  //       console.log({ bank: JSON.stringify(bank) });
  //       console.log({ CryptoPrice: JSON.stringify(CryptoPrice.toString()) });
  ...
  //   dexContract.on(
  //     'ClientOrder',
  //     (token, client, bank, CryptoPrice, date, event) => {
  //       let info = {
  //         token: token,
  //         client: client,
  //         bank: bank,
  //         CryptoPrice: formatUnits(CryptoPrice, 18),
  //         date: date,
  //         data: event,
  //       };
  //       console.log(JSON.stringify(info, null, 4));
  //       setNewBkTxId(JSON.stringify(info, null, 4));
  //       //  setNewBkTxId(JSON.stringify(info));
  //     }
  //   );
  // }
  ...
  async function getExchangeVoucherForId() {
    ...
    console.info('fetching Pay Cash transactions');
    const i = await dexContract.getExchangeVoucherForId(transactionData?.txId.toString());
  ...
  async function clientPayCash() {
    ...
    if (tx?.txId > 0) {
      ...
      try {
        ...
        let transaction = await dexContract.clientPayCash(tx?.txId);
