...
*/
  ...
  async function getCryptoOrder() {
    ...
    if (transactionData) {
      ...
      // Note: Always assign  service fee in string qouted value
      // let serviceFeeStr = dexShareStr;
      //=================={get managerWalletAddress after matching/connecting with manager}===================================
      //=================={get managerWalletAddress after matching/connecting with manager}===================================
  ...
  async function proceed() {
    ...
    await getCryptoOrder(); // after transaction, fetch latest transaction and set txId
  }
  ...
  async function getClientsTransacts() {
    ...
    // const transact = await dexContract.getClientsHasNotPaidCash();
    // const transact = await dexContract.getClientsHasNotPaidCash();
    const transact = await dexContract.getClientsTransacts(); // consider single transaction for all search
    ...
    // setTx(items[0]);
    // setTxId(items[0]?.txId);
    // setTx(items[1]);
    // setTxId(items[1]?.txId);
  ...
  const updateTransaction = async () => {
    ...
    if (transactionData && getExchangeVoucherForIdComplete) {
      const userData = {
        ...
        clientPaidCash: newTx?.clientPaidCash,
        bankReceivedCash: newTx?.bankReceivedCash,
        bankPaidCrypto: newTx?.bankPaidCrypto,
        clientReceivedCrypto: newTx?.clientReceivedCrypto,
      };
      const data = await updateTransactionById(userData);
  ...
  //     // userId: '64e5c829e9cfe950cb6d63f3',
  //     country: 'Russia',
  //     state: 'Moscow',
  //     city: 'Asino',
  //     walletAddress,
  //     chainId,
  //     // fromAddress: fToken?.address, // mainnet
  //     fromAddress: fToken.testAddress, // test net
  //     fromDecimals: fToken?.decimals,
  //     fromSymbol: fToken?.symbol,
  //     fromValue: fValue,
  //     // toAddress: tToken?.address,
  //     // toDecimals: tToken?.decimals,
  ...
  //     console.log({ userTransactionInfo: data });
  //     // setTransactionData(data);
  //     // recieve transaction submitted alert // Progress bar
  //   }
  // };
        ...
        {isConnected && !transferStatus.status && (
          <>
            {/* <div
              ...
              onClick={() => {
                proceed();
                localStorage.setItem('isActiveBlockchain', JSON.stringify(false));
              }}
            >
              <div
                className={`font-medium text-text-2-d ${stylesSwapTx.connectWallet}`}
  ...
  async function proceedClientPayCash() {
    if (!transactionData) {
      setTransferStatus({
        status: '309',
        message: 'One or more required fields are empty',
  ...
  async function clientPayCash() {
    ...
    if (transactionData?.txId > 0) {
      ...
      try {
        ...
        let transaction = await dexContract.clientPayCash(
          transactionData?.txId.toString()
        );
  ...
  async function clientReceiveCrypto() {
    if (tx?.txId > 0) {
      try {
        ...
        let transaction = await dexContract.clientReceiveCrypto(tx?.txId);
          ...
          {!isBlockchain && isActiveBlockchain ? (
            ...
            >
              <div className="flex flex-row justify-center items-center">
                <span className="flex font-medium text-[16px]">Delivery</span>
              </div>

              {/* <div className="flex flex-row justify-center items-center gap-2 rounded-lg bg-infoFill p-2 shadow-lg outline-none border">
                <div className="flex flex-col justify-center items-center gap-1 bg-primaryText rounded-lg h-[50px] p-2">
                <img className="h-6 w-6" src="/location-icon.svg" alt="" />
                <div className="flex font-medium text-[12px]">Previous</div>
                </div>
           
              <div className="flex flex-row justify-center items-center gap-1 bg-primaryText rounded-lg h-[50px] p-2">
              <div className="flex flex-row justify-center items-center bg-[var(--color-darkslategray-100)] rounded-lg">
                <div className=" ml-2 mr-2 flex font-medium text-[12px] text-white">Marata St, 24, ...</div>
                <div className="flex flex-col justify-center items-center gap-0.5">
                <img className="h-5 w-5 p-1 bg-[var(--color-gray-200-z)] rounded-r-lg" src="/delete-icon.svg" alt="" />
                <img className="h-5 w-5 p-1 bg-[var(--color-gray-200-z)] rounded-e-lg" src="/edit-icon.svg" alt="" />
                </div>
                </div> 
              ...
              <div className={stylesSwap.frameDiv}>
                ...
                <div className={`mt-6 ${stylesSwap.swapGroup}`}>
                  ...
                  >
                    <div className="flex flex-row justify-start items-center gap-1">
                      <img className="h-4 w-5" src="/phone-icon.svg" alt="" />
                ...
                <div className={`mt-6 ${stylesSwap.swapGroup}`}>
                  ...
                  >
                    <div className="flex flex-row justify-start items-center gap-1">
                      <img className="h-4 w-5" src="/comment-icon.svg" alt="" />
