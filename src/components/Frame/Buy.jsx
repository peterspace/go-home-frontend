...
const Buy = () => {
  ...
  useEffect(() => {
    if (isBuyCard || isSellCard) {
      setIsLaunching(true);
      setTimeout(() => {
        setIsLaunching(false);
      }, 3000);
    }
  }, [isBuyCard, isSellCard]);
        ...
        {isBuyCard ? (
          ...
          >
            ...
            <div className="flex flex-col  justify-center items-center gap-10 h-[684px] w-[440px] bg-white">
              ...
              <div
                className="mb-6 mt-4 cursor-not-allowed shadow-lg outline-none border gap-2 [border:none] w-[300px] py-6 px-3 rounded overflow-hidden flex flex-row items-center justify-center bg-infoFill"
                // onClick={() => {
                //   setIsBuyCard(false);
                //   setIsSellCard(true);
                // }}
              >
                <img className="h-6 w-6" src="/connect-wallet-90.png" />
                <div className={`font-bold text-2xl relative`}>Sell Crypto</div>
              </div>
            </div>
          </div>
        )}
